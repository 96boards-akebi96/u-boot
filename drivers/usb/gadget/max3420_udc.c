...
static uint8_t spi_rd8_ack(struct max3420_udc *udc, uint8_t reg, int actstat)
{
	struct spi_slave *slave = udc->slave;
...
static uint8_t spi_rd8(struct max3420_udc *udc, uint8_t reg)
{
	return spi_rd8_ack(udc, reg, 0);
}

static void spi_wr8_ack(struct max3420_udc *udc, uint8_t reg, uint8_t val, int actstat)
{
	struct spi_slave *slave = udc->slave;
	uint8_t txdata[2];
...
static void spi_wr8(struct max3420_udc *udc, uint8_t reg, uint8_t val)
{
	spi_wr8_ack(udc, reg, val, 0);
}

static void spi_rd_buf(struct max3420_udc *udc, uint8_t reg, void *buf, uint8_t len)
{
	struct spi_slave *slave = udc->slave;
	uint8_t txdata[1];
...
static void spi_wr_buf(struct max3420_udc *udc, uint8_t reg, void *buf, uint8_t len)
{
	struct spi_slave *slave = udc->slave;
	uint8_t txdata[1];
...
static int
max3420_ep_enable(struct usb_ep *_ep, const struct usb_endpoint_descriptor *desc)
{
...
{
	uint8_t epien, epirq, usbirq, usbien, reg[4];
	int ret = 0;
...
static int max3420_udc_probe(struct udevice *dev)
{
	...
	spi_get_bus_and_cs(3, 0, 10000000, SPI_MODE_3, "spi_generic_drv", NULL, &spid, &udc->slave);
